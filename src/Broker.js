...
class Playlist extends Model {
  ...
  #createRichSongModel = (anemiaSongModel) => {
    ...
    const richSongModel = this.#riches[songId] || new Song(anemiaSongModel)
    ...
    if (!this.#songProxy) {
      this.#songProxy = new Proxy(this.anemia.songs, {
        get: (target, name) => {
          ...
          return this.#createRichSongModel(target[name])
        }
      })
    }
...
export default class Broker extends EventEmitter {
  ...
  constructor() {
    super()
    let socket = this.#socket = socketIO.connect('//' + window.location.host, {
  ...
  get playlists () {
    ...
    return new Promise((rs) => {
      ...
      socket.once('playlists', (data) => {
        const playlists = this.#playlists = new Playlists(data.playlists)
        rs(playlists)
      })
      socket.emit('fetch_playlists')
    })
